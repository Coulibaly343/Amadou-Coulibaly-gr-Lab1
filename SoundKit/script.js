...
let playButtons = document.querySelectorAll(".sound");
assignButtons.forEach(item => {
...
trackBoxes.forEach(item => {
  TextTrackList.push(new Rec(item));
  item.querySelector(".track").recElement = trackList[trackList.length - 1];
  item.querySelector(".trackPlay").addEventListener("mousedown", rewind);
...
function btnPlay(e) {
  let audioElem =
    e.target.querySelector(".sounds") ||
    e.target.parentElement.querySelector(".sounds");
  playButtons(audioElem);
  e.target.classList.contains("soundBox")
    ? appendWave(e.target)
    : appendWave(e.target.parentElement);
}
...
function assignButton(e) {
  if (lookForBtn) {
    ...
    audioToAssign.parentElement.querySelector(".assignedButton").innerHtml =
      e.key;
    sounds.loadSounds();
  }
}
